...
SPECTRUM_EVERY = int(os.environ.get("TRACK3_SPECTRUM_EVERY", "1"))
SPECTRUM_START = int(os.environ.get("TRACK3_SPECTRUM_START", "1"))
...
def zeropower_via_newtonschulz5(G: Tensor) -> Tensor:
    ...
    # Polynomial polar iterations tuned from observed Track 3 Muon spectra.
    coeffs = [
        (8.246238042490457, -23.061595771559325, 16.607892376041224),
        (4.117891790804776, -2.8742027585235146, 0.5165966098909344),
        (4.112346184537485, -2.872784156567941, 0.5167827456555206),
        (4.089481953492347, -2.8669690836996296, 0.517570365043109),
        (3.9978773115772506, -2.8442535059839296, 0.5210732998956956),
        (3.620719259055243, -2.6561368875541955, 0.5120953471931012),
        (2.6646541959571466, -1.9428035783332864, 0.4332404008659599),
        (1.9450876286992687, -1.3066598912276628, 0.3660744608892639),
        (1.8566530431540582, -1.2134744594192381, 0.35682284456153723),
        (1.8564321588783959, -1.2132308528746187, 0.35679628415038767),
        (1.8564356435643565, -1.2132376849095554, 0.35679963285253075),
        (1.875, -1.25, 0.375),
    ]
    for a, b, c in coeffs:
        A = X @ X.mT
...
def maybe_log_spectrum(step: int, name: str, matrix: Tensor):
    if step < SPECTRUM_START or step % SPECTRUM_EVERY != 0:
        return
    X = matrix.detach()
...
print0(f"TRACK3_SPECTRUM_START={SPECTRUM_START}")
print0("="*100)
...
MUON_PARAM_NAMES = {id(p): name for name, p in model.named_parameters()}
